...
table2 = data.frame(Model = c('m1','m2','m2','m2','m3','m3','m3','m3'),
                    ...
                    Median = c(median(mcmc.m1.samples$samples$chain1[,1]),
                               median(mcmc.m2.samples$samples$chain1[,2]),
                               median(mcmc.m2.samples$samples$chain1[,3]),
                               abs(BPtoBCAD(round(median(mcmc.m2.samples$samples$chain1[,1])))),
                               median(mcmc.m3.samples$samples$chain1[,3]),
                               median(mcmc.m3.samples$samples$chain1[,4]),
                               median(mcmc.m3.samples$samples$chain1[,2]),
                               abs(BPtoBCAD(round(median(mcmc.m3.samples$samples$chain1[,1]))))),
