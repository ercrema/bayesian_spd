...
```{r s1_figure distribution map,fig.width=6,fig.height=7,echo=FALSE,warning = FALSE}
...
plot(basemap, col = "lightgrey", border = NA,xlim = c(129,133),ylim = c(30.9,34.3))
...
text(x=130.078,y=33.35701,"Nagasaki \n n=25",cex=1.5)
text(x=129.6752,y=32.97692,"Saga \n n=2",cex=1.5)
axis(1)
axis(2)
box()
...
```
\begin{center}
Figure S1: Geographic distribution of radiocarbon dates
\end{center}
\newpage


```{r s2_figure Experiment 4 Design,fig.width=6,fig.height=6,echo=FALSE}
a = 3450
b = 1850
true.mu = 2800
...
```
\begin{center}
Figure S2: Growth Models for Experiment 4
\end{center}
\newpage


```{r s3_figure prior predictive checks,fig.width=7.5,fig.height=3.5,echo=FALSE}
n = 500
set.seed(123)
...
```{r, s4_figure model 1 trace plot,fig.width=6,fig.height=6,echo=FALSE}
...
```{r, s5_figure Model 2 Trace Plots,fig.width=7.5,fig.height=6,echo=FALSE}
...
```{r, s6_figure Model 3 Trace Plots,fig.width=7.5,fig.height=7.5,echo=FALSE}
...
```{r, s_7 Temporal relation,echo=FALSE,fig.width=7,fig.height=7,echo=FALSE,message=FALSE,warning=FALSE}
...
axis(2)
box()
text(x=300,y=median(par('usr')[3:4]),label=paste('Changepoint after\n P=',round(sum(m2_diff>0)/length(m2_diff),2)),cex=0.8)
...
```
\begin{center}
Figure S7: Time difference between earliest dated evidence of charred rice in Kyushu (from Miyamoto 2018) and inferred change-point in growth rate for models 2 and 3. Differences were computed by comparing 15,000 random pairs of posterior samples from the two events. The highlighted region represents the 90% HPD density interval. Rice dates were calibrated using IntCal20 and combined using the R_Combine function in OxCal (via the OxCAAR R package).
\end{center}
\newpage