...
kyushu = tolower(names(c14samples))
kyushu[6]='oita'
df = data.frame(region=kyushu,value=as.numeric(c14samples)/sum(as.numeric(c14samples))*100)
admin1_choropleth('japan',df = df,legend='Percentage Samples',num_colors = 1,'',kyushu)
```
\begin{center}
...
```{r s2_figure,fig.width=6,fig.height=6,echo=FALSE}
a = 3450
b = 1850
true.mu = 2800
...
```{r s3_figure,fig.width=7,fig.height=4,echo=FALSE}
binsense(x=obs.caldates,y=obs.data$SiteID,calendar='BCAD',timeRange=c(a,b),h=c(0,50,100,200),verbose=FALSE)
```
\begin{center}
Figure S3: Bin Sensitivity Analysis
\end{center}
\newpage

```{r s4_figure,fig.width=7.5,fig.height=3.5,echo=FALSE}
n = 500
set.seed(123)
...
```
\begin{center}
Figure S4: Prior Predictive Checks
\end{center}
\newpage

```{r, s5_figure,fig.width=6,fig.height=6,echo=FALSE}
...
```
\begin{center}
Figure S5: Model 1 Trace Plots
\end{center}
\newpage

```{r, s6_figure,fig.width=7.5,fig.height=6,echo=FALSE}
...
```
\begin{center}
Figure S6: Model 2 Trace Plots
\end{center}
\newpage

```{r, s7_figure,fig.width=7.5,fig.height=7.5,echo=FALSE}
