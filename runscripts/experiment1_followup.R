...
modelPlot(model=dExponentialGrowth,a=a,b=b,params=list(r=true.r),type=c('spaghetti'),interval=0.9,calendar='BCAD',alpha=0.5)
  ...
  for (i in 1:nsim)
  {
    ...
    estimated.r[i]=median(mcmc.output$samples[,'r'])
    estimated.90hpd.lo[i]=HPDinterval(mcmc(mcmc.output$samples[,'r']),prob = 0.90)[1]
    estimated.90hpd.hi[i]=HPDinterval(mcmc(mcmc.output$samples[,'r']),prob = 0.90)[2]
  }

save(true.r,estimated.r,estimated.90hpd.lo,estimated.90hpd.hi,file=here('R_images','experiment1_followup_results.RData'))
